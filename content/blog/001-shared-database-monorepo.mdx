...
<CodeBlock lang="plaintext" filename="packages/db structure" code={`packages/db/
├── src/
│   ├── schema/
...
<CodeBlock lang="json" filename="packages/db/package.json" code={`{
  "name": "@workspace/db",
  "version": "0.0.1",
  "main": "src/index.ts",
  "types": "src/index.ts",
...
<CodeBlock lang="typescript" filename="packages/db/src/index.ts" code={`export { db } from "./client";
export * from "./schema";`} />
...
<CodeBlock lang="json" filename="apps/web/package.json (excerpt)" code={`"dependencies": {
